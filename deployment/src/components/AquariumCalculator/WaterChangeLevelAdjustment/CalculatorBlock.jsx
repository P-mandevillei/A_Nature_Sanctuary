...
export default function CalculatorBlock(props) {
    ...
    const storageItemName = `levelAdj${props.name}`;
    ...
    const [data, setData] = useLocalStorage(storageItemName, {'substance': t('name'), 'unit': t('unit'), 's': null, 'x': null, 'm': null, 'y': null});
    useEffect(()=>{
    ...
    const fields = {
        ...
        'x': {'name': t('x', {'name': data.substance}), 'unit': data['unit'], 'id': rocId, 'ref': xRef},
        'm': {'name': t('m', {'name': data.substance}), 'unit': data['unit'], 'id': targetId, 'ref': mRef},
        'y': {'name': t('y'), 'unit': '%', 'id': changeId, 'ref': yRef}
    }
    ...
    return <Card className="pad" style={{position: 'relative'}}> 
        ...
        <Form.Label htmlFor={rocId}>{t('x', {'name': data.substance})}: {data['unit']? `(${data['unit']})`:''}</Form.Label>
        ...
        <Form.Label htmlFor={changeId}>{t('y')} (%)</Form.Label>
